...
def main(
    ...
    max_new_tokens: int = typer.Option(600, help="Token budget per response."),
    ...
    reserved = [query.query for query in load_eval_queries(eval_file)]
    ...
    with typer.progressbar(length=len(prompts), label="Synthesising") as bar:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(futures):
                task = futures[future]
                try:
                    generated.extend(parse_synthesis_response(future.result(), task))
                except Exception:  # noqa: BLE001 - one dead request must not kill the run
                    failed += 1
                bar.update(1)

    kept, duplicates = deduplicate(generated, reserved)
