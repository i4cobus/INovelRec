...
#
# Usage:  bash scripts/19_serve_student.sh <checkpoint-dir> [PORT] [GPU]
set -euo pipefail

CKPT="${1:?usage: 19_serve_student.sh <checkpoint-dir> [port] [gpu]}"
PORT="${2:-8001}"
GPU="${3:-7}"
...
echo "Serving $CKPT on port $PORT (GPU $GPU)"
