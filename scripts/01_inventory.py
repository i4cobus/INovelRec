...
def main(
    ...
    summary.add_row("Near duplicates (same title+author)", str(duplicates.near_duplicates))
    ...
    summary.add_row("Lossy-decode recoveries", str(int((df["decode_replacement_chars"] > 0).sum()) if not df.empty else 0))
    summary.add_row("Unique novels for Stage 2", str(ok_count - duplicates.total_duplicates))
